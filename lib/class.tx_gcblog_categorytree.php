...
/**
 * generates a tree from gc_blog categories.
 *
 * $Id: class.tx_gcblog_categorytree.php 44560 2011-03-03 14:20:03Z rupi $
 *
 ...
 *
 */
class tx_gcblog_categorytree extends t3lib_treeview {
    ...
    function getBrowsableTree($groupByPages = false) {
        ...
        if ($this->gc_blog_obj->cache_categoryCount) {
            $storeKey = md5(serialize(array($this->stored,$this->MOUNTS,
                    $this->newsSelConf['pidInList'] . $this->newsSelConf['where'] . $this->gc_blog_obj->enableFields . $this->clause)));

//          $tmpCCC = $GLOBALS['TSFE']->sys_page->getHash($storeKey);
            $tmpCCC = $this->gc_blog_obj->cache->get($storeKey);
            if ($tmpCCC) {
                if ($this->gc_blog_obj->writeCachingInfoToDevlog>1) {
                    t3lib_div::devLog('categoryCountCache CACHE HIT (' . __CLASS__ . '::' . __FUNCTION__ . ')', 'gc_blog', - 1, array());
                }

                $this->categoryCountCache = unserialize($tmpCCC);
                $this->cacheHit = TRUE;
            } else {
                if ($this->gc_blog_obj->writeCachingInfoToDevlog) {
                    t3lib_div::devLog('categoryCountCache CACHE MISS (' . __CLASS__ . '::' . __FUNCTION__ . ')', 'gc_blog', 2, array($this->stored,$this->MOUNTS,
                    $this->newsSelConf['pidInList'] . $this->newsSelConf['where'] . $this->gc_blog_obj->enableFields . $this->clause));
                }
            }
        ...
        foreach ($this->MOUNTS as $idx => $uid) {
            ...
            if ($groupByPages) {
                $this->clause = $tmpClause . ' AND gc_blog_cat.pid=' . $uid;
                $rootRec['uid'] = 0;
            }
        ...
        if ($this->gc_blog_obj->cache_categoryCount && count($this->categoryCountCache) && !$this->cacheHit) {
//          $GLOBALS['TSFE']->sys_page->storeHash($storeKey, serialize($this->categoryCountCache), 'news_categoryCountCache');
            $this->gc_blog_obj->cache->set($storeKey,serialize($this->categoryCountCache),'categoryCounts');
        }
    ...
    function getNewsCountForCategory($catID) {
        ...
        if ($sum !== false) {
            //          t3lib_div::devLog('CACHE HIT (' . __CLASS__ . '::' . __FUNCTION__ . ')', 'gc_blog', - 1, array());
        } else {
            if ($this->gc_blog_obj->cache_categoryCount) {
                $hash = t3lib_div::shortMD5(serialize($catID . $this->newsSelConf['pidInList'] . $this->newsSelConf['where'] . $this->gc_blog_obj->enableFields . $this->clause), 30);
                $sum = $this->gc_blog_obj->cache->get($hash);

            }

            if ($sum === false) {
                if ($this->gc_blog_obj->writeCachingInfoToDevlog) {
                    t3lib_div::devLog('CACHE MISS (single count) (' . __CLASS__ . '::' . __FUNCTION__ . ')', 'gc_blog', 2, array());
                }
                ...
                $news_clause = '';
                if (is_object($this->gc_blog_obj)) {
                    $news_clause .= ' AND ' . $this->newsSelConf['where'] . $this->gc_blog_obj->enableFields;
                    if ($this->newsSelConf['pidInList']) {
                        $news_clause .= ' AND gc_blog.pid IN (' . $this->newsSelConf['pidInList'] . ') ';
                    }
                }

                tx_gcblog_div::getNewsCountForSubcategory($result, $catID, $news_clause, $this->clause);
                $sum = $result['sum'];

            }
            $this->categoryCountCache[$catID] = (int) $sum;
            if ($this->gc_blog_obj->cache_categoryCount) {
                $this->gc_blog_obj->cache->set($hash, (string)$sum,'categoryCounts');
            }
    ...
    function PMiconATagWrap($icon, $cmd, $isExpand = true) {
        if ($this->thisScript && $this->expandable) {
            ...
            $js = htmlspecialchars('txgcblogM1js.load(\'' . $cmd . '\', ' . intval($isExpand) . ', this, \'' . intval($this->pageID) . '\');');
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gc_blog/lib/class.tx_gcblog_categorytree.php']) {
    include_once ($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/gc_blog/lib/class.tx_gcblog_categorytree.php']);
}
?>